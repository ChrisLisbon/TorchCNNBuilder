...
def _validate_available_layers(
    ...
    minimum_feature_map_size: Tuple[int, ...],
) -> None:
...
def _validate_min_channels_number(input_channels_count_list, layer, transpose: bool = False) -> None:
    if transpose:
...
def _validate_build_transpose_convolve_init(
    in_channels: Optional[int], conv_channels: List[Tuple[int, ...], ...]
) -> None:
