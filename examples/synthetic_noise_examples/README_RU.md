...
Для отдельного обучения лучшей архитектуры вне сетки экспериментов, можно воспользоваться скриптом 
[```best_train.py```](best_train.py). Часть вывода данного скрипта для наглядности представлена в папке
[```best model```](best%20model).

Скрипты .py могут быть запущены следующей командой либо с помощью IDE.
``` 
python file_name.py
```
